...
<script type='text/javascript' src='{sugar_getjspath file='custom/include/javascript/layoutjs.js'}'></script>
...
{foreach from=$displayColumns key=colHeader item=params}
    ...
    {append var='colarray' value=$params.name index=$numb++}
  	
{/foreach}
{assign var="cname" value='@'|implode:$colarray}
...
{if $hideTable == false}
	...
	<tr height='20'>
			...
			{/foreach}
			<th class='td_alt' nowrap="nowrap" width='3%'><button  type="button" name="CustomListViewLayout" class="button" title="Customize columns viewed" style="float:right; background: url(custom/Source/images/columns.png);height: 25px;width: 25px;" id="CustomListViewLayout" value="Customize Columns" onclick="layoutset('{$cname}','{$currentModule}');" ></button></th>
		</tr>
	</thead>
		...
		{foreach name=rowIteration from=$data key=id item=rowData}
			...
			{if  $rowData.STAGE_C=="Marketing"} style="background-color:#F4FA58 !important;" {/if}
			{if  $rowData.STAGE_C=="Billing"} style="background-color:#A9F5A9 !important;" {/if}
			{if  $rowData.STAGE_C=="Hiring"} style="background-color:#81F7F3 !important;" {/if}
			{if  $rowData.STAGE_C=="Training"} style="background-color:#F5A9E1 !important;" {/if}
			{if  $rowData.STAGE_C=="Pre-Marketing"} style="background-color:#F6E3CE !important;" {/if}
			>
				{if $prerow}
