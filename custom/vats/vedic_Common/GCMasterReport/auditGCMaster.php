<?php
/**
  * FileName => auditGCMaster.php
  * Created By => Udaykiran(Created On Jun-21-2018)
  * Modified By => Udaykiran (Modified On Jun-21-2018)
  * COMMENT => Code to update the values in the audit table of GC
  */
...
$str = array();
	$bean = BeanFactory::getBean('vedic_GC', $gcID);
...
if (strpos($fieldName[0], 'vedic_gc') !== false) {
	...
	if (in_array($fieldName[1], $str))
	{
		...
		{	  
			...
			$query = $db->query("UPDATE vedic_gc
									SET 
									...
									WHERE id = '$gcID'");
			$id=create_guid();
			$auditMObile="INSERT INTO `vedic_gc_audit`(`id`,`parent_id`,`date_created`,`created_by`, `field_name`, `data_type`, `before_value_string`, `after_value_string`, `before_value_text`, `after_value_text`)
			VALUES ('".$id."',
					'".$gcID."',
					'".$CurrentDateTime."',
					'".$currentUser."',
