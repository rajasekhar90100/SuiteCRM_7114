<?php
/**
  * FileName => updateCandidate.php
  * Created By => Udaykiran (Created On Nov-21-2017)
  * Modified By => Lakshmi (Modified On Jan-09-2019)
  * COMMENT => Code to update the values in the audit table of profiles for Employee Active Report
  * COMMENT => Code to update the PO Enddate in the audit table of profiles for Employee Active Report
  */
...
$relateFields = array('ml_1_c','recruiter_c','primary_marketer_c','secondary_marketer_c','sales_c','hl_c','rl_c');
if (strpos($fieldName[0], 'vedic_profiles') !== false) {
	...
	if (in_array($fieldName[1], $str))
	{
	   if(($preValue[$fieldID])!=($postValue[$fieldID]))
		{	   
   			if(($fieldName[1] == 'stage_c')||($fieldName[1] == 'status_c')){   
				...
				$query = $db->query("SELECT CONCAT(COALESCE(TRIM(vedic_profiles.first_name),' '),' ',TRIM(vedic_profiles.last_name)) AS Profilename FROM vedic_profiles WHERE vedic_profiles.deleted=0 AND vedic_profiles.id ='$canID'");
				$result = $db->fetchByAssoc($query);
				$Profilename = $result['Profilename'];
				if($fieldName[1] == 'project_start_date_c')
				{
					$query =$db->query("SELECT vedic_candidates_vedic_profiles_1_c.`vedic_candidates_vedic_profiles_1vedic_candidates_ida` AS ID		
									...
									WHERE vedic_profiles.id = '$canID'		
									  AND vedic_candidates_vedic_profiles_1_c.deleted='0'");		
					$result = $db->fetchByAssoc($query);		
					$Can_ID = $result['ID'];
					if((empty($preValue[$fieldID])) && (!empty($postValue[$fieldID])))
					{
						if((empty($preValue['vedic_profiles_cstm.project_end_date_c'])) && (empty($preValue['vedic_profiles_cstm.channel_client_c'])))
						{
							...
							$project->vedic_profiles_project_1vedic_profiles_ida = $canID;
							$project->vedic_candidates_project_1vedic_candidates_ida = $Can_ID;
							$project->save();
						...
						else
						{
							if((!empty($preValue['vedic_profiles_cstm.project_end_date_c'])) || (!empty($preValue['vedic_profiles_cstm.channel_client_c'])))
							{
								...
								if($projectID!=''){
									...
									$project2->name = $Profilename;				
									$project2->estimated_start_date = $postValue[$fieldID];
									$project2->estimated_end_date = $postValue['vedic_profiles_cstm.project_end_date_c'];
									$project2->status = "Draft";
									$project2->vedic_profiles_project_1vedic_profiles_ida = $canID;
									$project2->vedic_candidates_project_1vedic_candidates_ida = $Can_ID;
									$project2->save();
								}
							}
						}
					}
					else if(($preValue[$fieldID]) && ($postValue[$fieldID]))
					{
						require_once('modules/vedic_Profiles/vedic_Profiles.php');
						$focus = new vedic_Profiles();
						$focus->retrieve($canID);
						...
						require_once('modules/Project/Project.php');
						if(($projectID!='')&&($postValue['vedic_profiles_cstm.project_start_date_c']!='')){
							$project = new Project();
							$project->retrieve($projectID);
							$project->estimated_start_date = $postValue[$fieldID];
							$project->save();
						}
						if(($postValue['vedic_profiles_cstm.project_start_date_c']!='')&&($projectID == '')){
							require_once('modules/Project/Project.php');
							$project2 = new Project();
							$project2->name = $Profilename;				
							$project2->estimated_start_date = $postValue[$fieldID];
							if($postValue['vedic_profiles_cstm.project_end_date_c']!=''){
								$project2->estimated_end_date = $postValue['vedic_profiles_cstm.project_end_date_c'];
							}
							$project2->status = "Draft";
							$project2->vedic_profiles_project_1vedic_profiles_ida = $canID;
				...
				else if($fieldName[1] == 'project_end_date_c')
				{
					if(($preValue[$fieldID])!=($postValue[$fieldID]))
					{
						require_once('modules/vedic_Profiles/vedic_Profiles.php');
						...
						if($projectID!=''){
							$project = new Project();
							$project->retrieve($projectID);
							$project->estimated_end_date = $postValue[$fieldID];
							$project->save();
						}
					}
				}
				...
				VALUES ('".$id."',
						...
						'')";
				$update_result = $db->query($auditMObile);
			}else if(($fieldName[1] == 'employee_type_c')){   
				require_once('modules/vedic_Profiles/vedic_Profiles.php');
				$prof = new vedic_Profiles();
				$prof->retrieve($canID);
				$projectID = $prof->vedic_profiles_project_1project_idb;
				require_once('modules/Project/Project.php');
				if($projectID!=''){
					$project1 = new Project();
					$project1->retrieve($projectID);
					$project1->w2ctc_c = $postValue[$fieldID];
					$project1->save();
				}
			...
			else if(($fieldName[1] == 'po_end_date_c')){   
				require_once('modules/vedic_Profiles/vedic_Profiles.php');
				$prof = new vedic_Profiles();
				$prof->retrieve($canID);
				$profilePOEndDate = $prof->po_end_date_c;
			    $prPOEndDate = date_format(date_create($profilePOEndDate),"Y-m-d");
				$projectID = $prof->vedic_profiles_project_1project_idb;
				require_once('modules/Project/Project.php');
				if($projectID!=''){
					$project1 = new Project();
					$project1->retrieve($projectID);
					$projectPOEndDate = $project1->po_enddate;
				    $proPOEndDate = date_format(date_create($projectPOEndDate),"Y-m-d");
					if(strtotime($profilePOEndDate)!= strtotime($projectPOEndDate)) {
					   $project1->po_enddate = $postValue[$fieldID];
				    }
					$project1->save();
				}
				$id=create_guid();
...
else if ((strpos($fieldName[0], 'pro') !== false) || (strpos($fieldName[0], 'prc') !== false)) {	
	if($canID)
	{
