   ...
   ********************/
        ...
        _updatePlaceholder: function() {
            ...
            if (window.intlTelInputUtils && shouldSetPlaceholder && this.selectedCountryData) {
				...
				//Changed the placeholders values like ex: +1 XXX-XXX-XXXX modified by Udaykiran on 22-Sept-2017
				var arr = ['as','ai','ag','bs','bb','bm','vg','ky','dm','gd','gu','jm','ms','mp','vc','lc','kn','sx','tt','tc','vi'];
				var place,countryCode;
				if((jQuery.inArray( this.selectedCountryData.iso2, arr ))!=-1)
				{
					place = placeholder.substr(placeholder.indexOf('-')+1);
					countryCode = this.selectedCountryData.dialCode;
					countryCode = countryCode.substr(0,1).concat(' ').concat(countryCode.substr(1,3));
				}else{
					place = placeholder.substr(placeholder.indexOf(' ')+1);
					countryCode = this.selectedCountryData.dialCode;
				}
				placeholder = place.replace(/[0-9]/g, "X");
				placeholder = "+"+countryCode.concat(' ').concat(placeholder);
